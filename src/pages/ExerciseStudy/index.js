                        ...
                        <div className="flex flex-col w-1/5 bg-lime-100 min-h-screen border-r-2 border-black">
                            ...
                            {/* Các bài học con */}
                            {
                                lecturessArray.map((lecture) => (
                                    <button onClick={() => setLessons(lecture)} key={lecture.id} className={`p-3 text-normal text-indigo-700 font-bold hover:bg-amber-400 focus:bg-amber-500 border ${lesson.id === lecture.id ? 'bg-amber-500' : 'bg-lime-100'}`}>{lecture.title}</button>
                                ))
                            }
                        </div>

                        <div className="w-4/5 bg-lime-100 min-h-screen">
                            <BlocklySpace IdLectures={IdLectures} lesson={lesson} />
                        </div>
                    </div>
