...
function BasicEx({userInfo}) {
    ...
    const handleClick = useCallback((lecture) => {
        ...
        newBlock.setFieldValue(lecture.contentBs, 'QUESTION');
    ...
    const saveCompletedLecture = async (user, lectureId, title) => {
        ...
        const score = 15;
console.log(user)
        await set(child(dbRef, `accounts/${user.replace("User","")}/completedLectures` + `/basic/${lectureId}`), {
            title,
            completeAt,
            score,
        }).catch((error) => {
    ...
    useEffect(() => {
        const fetchCompletedLectures = async () => {
            const snapshot = await get(child(dbRef, `accounts/${user.id.replace("User","")}/completedLectures/basic`));
            if (snapshot.exists()) {
